...
export default function CursoDetalhes({
      ...
      try {
        ...
        if (coursesResult.success && coursesResult.data) {
          const foundCourse = coursesResult.data.find(
            (c) =>
              criarSlug(c.title) === cursoId &&
              c.category_id === parsedCategoryId
          );
          ...
          if (foundCourse) {
            ...
            if (modulesResult.success && modulesResult.data) {
              const courseMod = modulesResult.data
                .filter(
                  (m) => m.course_id === foundCourse.course_id
                )
                .sort(
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <Loader2
          className="animate-spin text-[#046279]"
          size={32}
        />
      </div>
    );
    ...
    return (
      <div className="p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <Link
            href={`/cursos/${categoryId}`}
            className="text-[#046279] font-semibold hover:underline"
          >
            Voltar para a categoria
          </Link>
        </div>

        <h1 className="text-xl font-bold text-red-500 mt-4">
          Curso não encontrado
        </h1>

        <p className="mt-2 text-gray-600">
          Verifique se o link está correto ou escolha outro curso.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      ...
      <h1 className="text-3xl font-bold text-[#046279] mt-4">
        {course.title}
      </h1>
            ...
            >
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm hover:shadow-lg hover:border-[#046279] transition cursor-pointer">
                <div className="flex items-center gap-3">
                  <Play
                    className="text-[#046279] flex-shrink-0"
                    size={20}
                  />
